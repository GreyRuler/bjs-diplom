...
const ratesCurrency = () => ApiConnector.getStocks(
    response => {
        if (response.success) {
...
ratesCurrency();
setInterval(ratesCurrency, 60000);
...
moneyManager.addMoneyCallback = data => ApiConnector.addMoney(
    ...
    response => {
        if (response.success) {
            ProfileWidget.showProfile(response.data);
            moneyManager.setMessage(response.success, "Пополнено");
        } else {
...
moneyManager.conversionMoneyCallback = data => ApiConnector.convertMoney(
    ...
    response => {
        if (response.success) {
            ProfileWidget.showProfile(response.data);
            moneyManager.setMessage(response.success, "Конвертируемо")
        } else {
            moneyManager.setMessage(response.success, response.error);
        }
    }
...
moneyManager.sendMoneyCallback = data => ApiConnector.transferMoney(
    ...
    response => {
        if (response.success) {
            ProfileWidget.showProfile(response.data);
            moneyManager.setMessage(response.success, "Переведено");
        } else {
            moneyManager.setMessage(response.success, response.error);
        }
    }
...
favoritesWidget.removeUserCallback = data => ApiConnector.removeUserFromFavorites(
    data,
    response => {
        if (response.success) {
            favoritesWidget.clearTable();
